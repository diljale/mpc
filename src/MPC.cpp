  ...
  void operator()(ADvector& fg, const ADvector& vars) {
   ...
   // The cost is stored is the first element of `fg`.
    ...
    for (size_t t = 0; t < N - 1; t++) {
      ...
      fg[0] += 8*CppAD::pow(vars[a_start + t], 2);
    }
...
vector<double> MPC::Solve(Eigen::VectorXd x0, Eigen::VectorXd coeffs) {
  ...
  for (size_t i = delta_start; i < delta_start + latency_step; i++) {
 ...
 for (size_t i = a_start; i < a_start+latency_step; i++) {
  ...
  return {solution.x[x_start + latency_step],   solution.x[y_start + latency_step],
          ...
          solution.x[delta_start  + latency_step],   solution.x[a_start + latency_step]};
